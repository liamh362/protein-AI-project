...
st.session_state.mutated_sequence = st.text_input("Enter a mutated version of your sequence (optional):", st.session_state.mutated_sequence)

async def analyze_sequence(sequence):
    """Analyze a protein sequence using real tools."""
    return await st.session_state.protein_analyzer.analyze_protein(sequence)
    ...
    if not sequence:
        ...
        if invalid_chars:
            ...
            with st.spinner("Analyzing protein sequence... This may take a few moments."):
                ...
                analysis_results = asyncio.run(analyze_sequence(sequence))
...
if st.button("Compare Mutation"):
    ...
    if not mutated_sequence:
        ...
        with st.spinner("Analyzing mutation effects..."):
            ...
            mutated_results = asyncio.run(analyze_sequence(mutated_sequence))
